...
| **App** | `src/index.html` | Leaflet-Karte + MarkerCluster (beide **lokal in `src/vendor/`, inline eingebettet** seit 2026-08-31 / DSGVO — kein unpkg-CDN) + Filter + Detail-Popups + **Statistik-Panel (6 Tabs)** + **2-Klick-Consent für OSM-Kacheln**. **V5:** Responsive Design (3 Breakpoints). **V6:** Art-Verteilungs-Pie unter Größenklassen. **V7/V7b/V7c:** Registrierungs-Filter (Jahr/Monat), Alle-Anlagen-Tabelle mit sortierbaren Headern. **V8:** Zubau-Tab mit 6 Charts (Stacked Bar, Bar+Trend, Heatmap, Raten, Kumuliert). |
